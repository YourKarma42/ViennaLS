...
/// guidance)
template <class T, int D> class eulerAdvect {
...
public:
  ...
  eulerAdvect(lsSmartPointer<lsDomain<T, D>> passedlsDomain) {
  ...
  template <class VelocityField>
  eulerAdvect(lsSmartPointer<VelocityField> passedVelocities) {
    velocities =
        std::dynamic_pointer_cast<lsVelocityField<T>>(passedVelocities);
  }

  template <class VelocityField>
  eulerAdvect(std::vector<lsSmartPointer<lsDomain<T, D>>> passedlsDomains,
           lsSmartPointer<VelocityField> passedVelocities)
